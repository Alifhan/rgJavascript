function gameGrouping(games) {
  if (games.length <= 1) {
    ...
    function groupArr(array, key, keyValues) {
      return array.reduce((objek, data) => {
        if (objek[data[key]]?.[keyValues]) {
          objek[data[key]][keyValues].push(data[keyValues]);
        } else {
          objek[data[key]] = { [keyValues]: [data[keyValues]] };
        }
        return objek;
      }, {});
    }
